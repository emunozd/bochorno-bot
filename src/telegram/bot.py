...
Same auth flow as bochorno-bot (seed phrase vinculation).
Commands and alerts adapted to weather/temperature context.
"""
...
def init(state: dict, lock: threading.Lock) -> None:
    ...
    _state = state
    _lock  = lock
    # Give engine.py a reference to the lock for thread-safe state updates
...
async def cmd_help(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    txt = (
        ...
        "*Control*\n"
        "  /close BUENOS\\_AIRES — Cierre forzado\n"
...
async def cmd_signals(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    for city_key, sig in signals.items():
        ...
        name = _e(cfg.get("name", city_key))
        unit = sig.get("unit", "C")
        wcs  = sig.get("wcs", 0)
        zone = _e(sig.get("wcs_zone", "—"))
        ...
        std_str = f"±{std:.1f}°{unit}" if std is not None else ""
        ...
        lines.append(
            f"🌡 *{name}*\n"
            f"  T\\_pred: {_e(T_str)} {_e(std_str)}\n"
            f"  Outcome: {_e(bo_str)} | PIP: {_e(pip_str)} | Mkt: {_e(mkt_str)} | Edge: {_e(edge_str)}\n"
            f"  WCS: {_e(f'{wcs:.1f}')}/100 \\({zone}\\)\n"
            f"  {_e(model_line)}\n"
...
async def cmd_positions(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    for city_key, pos in positions.items():
        ...
        lines.append(
            f"*{name}* — {_e(f'{pos.outcome_val}°{unit}')} YES\n"
            f"  💰 {pos.shares:.2f} shares @ ${_e(f'{pos.entry_price:.3f}')}\n"
            f"  📥 ${_e(f'{pos.usdc_spent:.2f}')} → ${_e(f'{cur_val:.2f}')}\n"
            ...
            f"  🗓 {_e(entry_dt)} ET  WCS:{pos.entry_wcs:.0f}  PIP:{pos.entry_pip:.3f}\n"
        )
...
async def cmd_portfolio(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    txt = (
        ...
        f"  Trades: {len(recent)}   Win rate: {_e(f'{win_rate:.0f}')}%"
...
async def cmd_trades(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    for t in recent:
        ...
        lines.append(
            f"{col} {_e(cfg.get('name', t.get('city_key','?')))} "
            f"{_e(str(t.get('outcome_val','?')))}°{unit} "
            f"— {_e(_fmt_pnl(t.get('pnl',0)))} USDC "
            f"\\({_e(t.get('reason','?'))}\\)"
        )
...
async def cmd_status(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    txt = (
        ...
        f"  🔄 Fetching: {_e(', '.join(fetching) or 'idle')}\n"
        f"  📡 Última señal: {_e(ls)}\n"
        f"  📥 Último fetch: {_e(lu)}\n"
...
async def cmd_close(update: "Update", ctx: "ContextTypes.DEFAULT_TYPE") -> None:
    ...
    if not pos:
        await update.message.reply_text(
            f"📭 No hay posición abierta para {_e(city_key)}\\.",
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return

    # Use last known market price or entry price as fallback
    with _lock:
...
def alert_position_opened(city_key, outcome_val, unit,
    ...
    std_str = f"±{T_std:.1f}°{unit}" if T_std is not None else ""
    _send_sync(
        f"🚀 *Posición Abierta*\n\n"
        f"  🌡 *{name}* — {_e(f'{outcome_val}°{unit}')} YES\n"
